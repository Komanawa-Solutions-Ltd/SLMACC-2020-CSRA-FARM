...
name = 'historical_quantified_1yr'
...
def export_and_plot_data():
    export_all_in_pattern(base_outdir=outputs_dir,
                          patterns=[
                              ...
                              os.path.join(os.path.dirname(base_pg_outdir), 'baseline_sim_no_pad', '*.nc') #todo problem
                          ])
    for sm in ['eyrewell-irrigated', 'oxford-dryland', 'oxford-irrigated']:
        ...
        for p in paths:
            ...
            if not os.path.exists(outdir):
                os.makedirs(outdir)
            data_paths = [p,
                          os.path.join(os.path.dirname(base_pg_outdir), 'baseline_sim_no_pad', f'0-baseline-{sm}.nc')] # todo problem
