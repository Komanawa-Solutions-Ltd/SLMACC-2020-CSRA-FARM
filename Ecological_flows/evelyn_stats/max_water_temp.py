...
daily_max_df = pd.read_csv(water_base_path)
...
#daily_max_df['Date & Time'] = pd.to_datetime(daily_max_df['Date & Time'], format='%Y/%m/%d')
#daily_max_df = daily_max_df.rename(columns={'Date & Time' : 'date', 'Water Temp (degC)': 'daily_max_water_temp'})
#merged_df = daily_max_df.merge(mean_air_temp_df)
#merged_df.to_csv(kslcore.KslEnv.shared_gdrive.joinpath("Z2003_SLMACC/eco_modelling/temp_data/waiau_temp/air_water_max_temp_data.csv"))
#
